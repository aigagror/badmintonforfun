            ...
            for party in parties:
                ...
                for i in range(len(response_parties)):
                    if response_parties[i]['party_id'] == party.id:
                        curr_party_avg_play_time = round(response_parties[i]['avg_time'], 3)  # in seconds
                        break

                party_dict['average_play_time'] = round(curr_party_avg_play_time/60, 3)
...
def get_parties_by_playtime(queue_type):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute(query_full_playtime, [queue_type])
